...
# ORB_Live_Trader/backtest/engine.py -> ORB_Live_Trader/ -> Project Root
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_DIR = PROJECT_ROOT / "data"

# Default paths for backtest data
...
def run_strategy(
    ...
    if start_date or end_date:
        ...
        print(f"  After date filter: {len(df_universe):,}")
    
    df_filtered = df_universe[
        (df_universe['atr_14'] >= min_atr) &
        (df_universe['avg_volume_14'] >= min_volume)
    ].copy()
    print(f"  After runtime filters: {len(df_filtered):,}")
    ...
    print(f"Simulating trades (Stop: {stop_atr_scale*100:.1f}% ATR, mode={mode_str})...")
...
def main():
    ...
    # Simple defaults for args possibly passed but not explicitly parsed here to keep it simple
    # Requires universe path relative to standard orb universe dir
    
    universe_path = ORB_UNIVERSE_DIR / args.universe
    if not universe_path.exists():
        legacy_path = DATA_DIR / "backtest" / args.universe # fallback
