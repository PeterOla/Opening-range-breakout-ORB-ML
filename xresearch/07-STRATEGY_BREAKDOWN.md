...
Two validated sentiment-driven Opening Range Breakout strategies that eliminate look-ahead bias whilst maintaining exceptional performance (+113,903% over 2 years from $1.5K seed capital).

**Core Concept**: Use positive news sentiment to filter micro-cap universe, then trade ORB breakouts on high RVOL candidates.

---

## Strategy 1: Rolling 24-Hour Window
...
- **Other thresholds tested**: 0.60, 0.70, 0.80, 0.95
...
## Strategy 2: Pre-Market Only

### Full Workflow (Step-by-Step)

#### Differences from Rolling 24H

Only **Step 3 (News Attribution)** differs. All other steps identical.

#### 3. News Attribution (Pre-Market Logic)
- **Timezone conversion**: UTC timestamps → America/New_York
- **Attribution rule**: News available from **midnight to 09:30 today** → trade today
- **Implementation**:
  ```python
  if news_time < 09:30:
      trade_date = news_date  # Pre-market only (00:00-09:30)
  else:
      trade_date = news_date + 1 business day  # Everything else → next day
  ```
- **Examples**:
  - News Monday 07:00 AM ET → trade Monday (pre-market) ✅
  - News Monday 14:00 PM ET → trade Tuesday (market hours → shifted) ✅
  - News Sunday 18:00 PM ET → trade Monday (weekend → shifted to next business day) ✅
- **Captures**: Only same-day pre-market news (more conservative)
- **Excludes**: Overnight news from previous day (unlike Rolling 24H)
- **No look-ahead**: Market-hours news cannot influence same-day entry

#### Why Pre-Market and Rolling 24H Produce Identical Results
- **Same candidates**: Both shift market-hours news forward by 1 day
- **Same universe size**: 4,934 rows for 0.90 threshold
- **Same trades**: 857 total, 706 entered
- **Convergence**: After deduplication by `[trade_date, ticker]`, overnight news from yesterday doesn't create additional candidates — either:
  1. Ticker already has pre-market news today (higher score wins), or
  2. Ticker has no pre-market news, so overnight adds it (same trade_date)
- **Practical implication**: Use Rolling 24H for conceptual completeness, but expect identical backtest results
...
|---|---|---|---|
| Final Equity | $1,708,048 | $413,152 | **-76%** |
| Win Rate | 12.7% | 16.7% | +4.0pp |
| Profit Factor | 2.49 | 1.49 | **-40%** |
| Return | +113,903% | +27,444% | **-76%** |
...
### Comparison vs Look-Ahead Biased
- **Look-ahead (inflated)**: $62.7M, 18.2% WR, 12.62 PF
- **Fixed 5% ATR (realistic)**: $1.7M, 12.7% WR, 2.49 PF
- **Fixed 10% ATR (wider)**: $413K, 16.7% WR, 1.49 PF
- **Deflation**: -97.3% (5% stop) / -99.3% (10% stop)
...
### Scripts
1. **Fetch news**: `prod/backend/scripts/research/fetch_full_universe_news.py`
   - Fetches historical news from Alpaca API
   - Handles multi-symbol attribution correctly

2. **Score sentiment**: `prod/backend/scripts/research/score_sentiment.py`
   - Loads FinBERT model
   - Scores headlines in batches
   - Outputs scored parquet

3. **Enrich universe**: `prod/backend/scripts/research/enrich_sentiment_universe.py`
   - Applies attribution logic (`--mode rolling_24h` or `--mode premarket`)
   - Enriches with price data, OR metrics, RVOL
   - Outputs tradeable universe

4. **Backtest**: `prod/backend/scripts/ORB/fast_backtest.py`
...
Raw News (Alpaca API)
    ↓
news_micro_full_1y.parquet (75,760 items)
    ↓ [FinBERT scoring]
news_micro_full_1y_scored.parquet (75,760 items + scores)
    ↓ [Attribution + enrichment]
universe_sentiment_0.9.parquet (4,934 candidates)
    ↓ [Backtest simulation]
simulated_trades.parquet (857 trades)
daily_performance.parquet (231 days)
yearly_results.parquet (2 years)
```
